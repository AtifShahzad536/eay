      ...
      <div className="max-w-[1600px] mx-auto px-8 py-20 relative">
        ...
        <div className="flex flex-col mb-24">
          ...
          <div className="relative">
            <h1 className="text-6xl md:text-[5.5rem] font-black text-gray-900 tracking-tighter leading-[0.75] mb-8 uppercase">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-20 p-8 ">

          {/* Fabric Patterns */}
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-2 text-[10px]  text-gray-900 uppercase tracking-widest">
          ...
          <div className="flex flex-col gap-4">
            ...
            <button onClick={() => setMouseFollow(!mouseFollow)} className={`w-full py-5 rounded-md text-[9px]  uppercase tracking-widest border transition-all ${mouseFollow ? 'bg-blue-600 text-white border-blue-600 shadow-lg shadow-blue-500/20' : 'bg-white text-gray-400 border-gray-200'}`}>
        ...
        <div className="flex items-center gap-4 mb-20 overflow-x-auto pb-4 scrollbar-hide">
          <span className="text-[10px]  text-gray-300 uppercase tracking-[0.4em] whitespace-nowrap mr-6">Designer Presets:</span>
          {themes.map(t => (
            <button key={t.name} onClick={() => applyTheme(t)} className="flex items-center gap-3 px-6 py-3 bg-white border border-gray-100 rounded-lg hover:border-blue-500 transition-all group shrink-0 shadow-sm hover:shadow-md">
              ...
              <span className="text-[10px]  text-gray-900 uppercase tracking-widest group-hover:text-blue-600">{t.name}</span>
            </button>
          ))}
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-20">
            <div className="flex items-center gap-6">
              ...
              <h2 className="text-4xl  text-gray-900 uppercase tracking-tighter">Elite Gallery</h2>
            </div>
            {comparing.length > 0 && (
              <div className="flex items-center gap-4 bg-blue-50 px-4 py-2 rounded-lg border border-blue-100 animate-fade-in">
                <span className="text-[9px]  text-blue-600 uppercase tracking-widest">Compare Mode ({comparing.length}/2)</span>
              ...
              <div key={design.id} className={`group cursor-pointer transition-all duration-500 ${comparing.includes(design.id) ? 'scale-105' : ''}`}>
                ...
                >
                  ...
                  <div className="absolute top-4 left-4 flex gap-2">
                    <span className="px-2 py-1 bg-white/80 backdrop-blur-md rounded-md text-[8px]  text-gray-400 border border-gray-100 uppercase tracking-widest">{design.id}</span>
                  </div>
                  ...
                  <div className="absolute bottom-6 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                    <div className="flex items-center gap-3 bg-gray-900 text-white px-6 py-2.5 rounded-md shadow-2xl">
                      <span className="text-[9px]  uppercase tracking-widest">Customize</span>
                      <HiArrowRight className="text-xs" />
                    </div>
                  </div>
                </div>
                <div className="mt-6 flex flex-col gap-1 px-1" onClick={() => onSelectDesign(design)}>
                  <h3 className="text-[14px]  text-gray-900 uppercase tracking-tighter group-hover:text-blue-600 transition-colors">{design.name.split(' / ')[0]}</h3>
                  <div className="flex items-center justify-between">
                    <span className="text-[9px]  text-gray-400 uppercase tracking-[0.2em]">{design.name.split(' / ')[1]}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="mt-40 pt-16 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-10">
          <div className="flex items-center gap-10">
            <div className="flex flex-col gap-1"><span className="text-[9px]  text-gray-300 uppercase tracking-widest">Engine</span><span className="text-[11px]  text-gray-900 uppercase">GPU Matrix v4.0</span></div>
            <div className="flex flex-col gap-1"><span className="text-[9px]  text-gray-300 uppercase tracking-widest">Render</span><span className="text-[11px]  text-gray-900 uppercase">High Fidelity</span></div>
          </div>
          <div className="text-[10px]  text-gray-200 uppercase tracking-[0.5em]">EAY Studio — PRO EDITION</div>
        </div>
      </div>
    </div >
  );

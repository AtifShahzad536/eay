    ...
    <div className="flex flex-col bg-white">
      ...
      {imageDecals.length > 0 && (
        <div className="p-6 bg-white border-b border-gray-50">
          ...
          {selected && (
            <div className="mt-6 p-4 bg-gray-50 rounded-none border border-gray-100">
              <div className="flex items-center justify-between mb-3">
                <p className="text-[9px] font-semibold text-gray-400 uppercase tracking-widest">Component Scale</p>
                <span className="text-[10px] font-semibold text-blue-600">{((selected.decalScale || 0.12) * 100).toFixed(0)}%</span>
              </div>
              <input type="range" min="0.03" max="0.3" step="0.01" className="w-full h-1.5 bg-gray-200 rounded-none appearance-none cursor-pointer accent-blue-600" value={selected.decalScale || 0.12} onChange={(e) => safeUpdate({ decalScale: parseFloat(e.target.value) })} />
            </div>
          )}
        </div>
      )}
